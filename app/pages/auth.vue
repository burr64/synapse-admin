...
<template>
  ...
  <div
    class="flex items-center justify-center md:rounded-2xl h-screen md:h-full bg-primary"
  >
      ...
      <form @submit.prevent="login" class="pb-6 flex flex-col items-center gap-8">
        ...
        <div class="w-full flex flex-col gap-4 [&_p]:text-sm [&_p]:pb-1">
          <UFormField :label="$t('username')" required>
            <UInput v-model="username" id="username" icon="material-symbols:account-circle" size="xl" variant="soft" class="w-full bg-gray-100 rounded-2xl border-2 border-transparent focus-within:border-2 focus-within:border-primary overflow-hidden"/>
          </UFormField>
          <UFormField :label="$t('password')" required>
            <UInput icon="material-symbols:password" v-model="password" size="xl" :type="show ? 'text' : 'password'" :ui="{ trailing: 'pe-1' }" variant="soft" class="w-full bg-gray-100 rounded-2xl border-2 border-transparent focus-within:border-2 focus-within:border-primary overflow-hidden">
              <template #trailing>
                <UButton
                  color="neutral"
                  variant="link"
